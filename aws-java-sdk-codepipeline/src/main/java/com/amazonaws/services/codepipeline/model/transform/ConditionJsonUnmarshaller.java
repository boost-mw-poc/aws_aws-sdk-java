        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("result", targetDepth)) {
                    context.nextToken();
                    condition.setResult(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("rules", targetDepth)) {
                    context.nextToken();
                    condition.setRules(new ListUnmarshaller<RuleDeclaration>(RuleDeclarationJsonUnmarshaller.getInstance())
