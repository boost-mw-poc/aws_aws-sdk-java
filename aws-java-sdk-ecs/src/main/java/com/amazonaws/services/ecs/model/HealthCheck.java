 ...
 * transition to an <code>UNHEALTHY</code> status. This is by design, to ensure that containers remain running during
 * agent restarts or temporary unavailability. The health check status is the "last heard from" response from the Amazon
 * ECS agent, so if the container was considered <code>HEALTHY</code> prior to the disconnect, that status will remain
 * until the agent reconnects and another health check occurs. There are no assumptions made about the status of the
 * container health checks.
 * </p>
 * </li>
 * <li>
 * <p>
 * Container health checks require version <code>1.17.0</code> or greater of the Amazon ECS container agent. For more
 * information, see <a href="https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-agent-update.html">Updating
