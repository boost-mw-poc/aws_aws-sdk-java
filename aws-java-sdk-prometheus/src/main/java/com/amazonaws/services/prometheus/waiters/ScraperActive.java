...
class ScraperActive {
    ...
    static class IsACTIVEMatcher extends WaiterAcceptor<DescribeScraperResult> {
    ...
    static class IsCREATION_FAILEDMatcher extends WaiterAcceptor<DescribeScraperResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"CREATION_FAILED\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }
