...
class ScraperDeleted {
    ...
    static class IsResourceNotFoundExceptionMatcher extends WaiterAcceptor<DescribeScraperResult> {
        /**
    ...
    static class IsDELETION_FAILEDMatcher extends WaiterAcceptor<DescribeScraperResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"DELETION_FAILED\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }

        private static final JmesPathExpression ast = new JmesPathSubExpression(new JmesPathField("scraper"), new JmesPathField("status"), new JmesPathField(
                "statusCode"));
         ...
         */
        @Override
        public boolean matches(DescribeScraperResult result) {
